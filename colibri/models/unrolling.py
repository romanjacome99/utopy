...
from colibri.recovery.fista import Fista, ProgressiveFista
...
class UnrollingFISTA(Fista):
    ...
    def forward(self, y, freq=0, x0=None):
        '''
        ...
        return super(UnrollingFISTA, self).forward(y, x0=x0, freq=freq)
    

    

    
class UnrollingProgressiveFISTA(ProgressiveFista):
    ...
    def __init__(self, acquistion_model, fidelity=None, max_iters=5, models=None, alpha=1e-3, _lambda=0.1):
        ...
        Returns:
            None
        '''
        super(UnrollingProgressiveFISTA, self).__init__(acquistion_model=acquistion_model,fidelity=fidelity, max_iters=max_iters, alpha=alpha, _lambda=_lambda)

        self.fidelity = fidelity
        self.acquistion_model = acquistion_model

        self.H = [acquistion_model[0].forward,acquistion_model[1].forward]
        ...
        for _ in range(self.max_iters):
            ...
            t_squence.append((t_old -1)/t)
        self.t_squence = nn.ParameterList( [nn.Parameter(torch.tensor(t),requires_grad=True) for t in t_squence])
        self.prior = LearnedPrior(models=models)

    def forward(self, y, freq=0, x0=None,gamma=0.0):
        '''
        Runs the unrolling algorithm to solve the optimization problem.
        ...
        return super(UnrollingProgressiveFISTA, self).forward(y, x0=x0, freq=freq,gamma=gamma)
