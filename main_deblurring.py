...
def main(args):
    ...
    wandb.login(key='key-wandb')  # Replace with your actual WandB key
    set_seed(args.seed)
    ...
    exp_name = f"CAMSAP_deblurr_sigma_ag{args.sigma_ag}_sigma_a_{args.sigma_a}_type_Ag_{args.type_Ag}_iters={args.max_iter}_dataset={args.dataset}_freq={args.freq}_type_run={args.type_run}_n={args.n}_batch_size={args.batch_size}_lr={args.lr}_epochs={args.epochs}_lr={args.lr}_param_continuation={args.param_continuation}"       
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=int(args.epochs/3), gamma=0.5)
    ...
    wandb.init(project="CAMSAP_UTOPY", name=exp_name, config=args, entity='entity')  # Replace 'entity' with your actual WandB entity
    ...
    for iter in range(args.epochs):
        if args.type_run == 'baseline':
            ...
            if iter % args.freq == 0 and iter != 0:
        ...
        for i, data in enumerate(data_loop):
            ...
            x0 = alpha * blur_t(yt, type_calculation="backward") + (1 - alpha) * blur(y, type_calculation="backward")
        ...
        if iter == 0:
            ...
            continuation_path = mse(x_hat, x_old).item()
            x_old = x_hat
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    #----------------------------------- Sensing parameters ---------------------------------
    parser.add_argument('--sigma_a', type=float, default=5, help='SIgma of the Gaussian kernel for sensing matrix A')
    parser.add_argument('--sigma_ag', type=float, default=1, help='SIgma of the Gaussian kernel for sensing matrix Ag')
    #----------------------------------- FISTA parameters -----------------------------------
    ...
    parser.add_argument('--alpha_scheduler', type=str, default='exponential', choices=['exponential', 'linear'], help='Scheduler type for alpha parameter (exponential or linear)')
